...
function DeepseekInput({setNewConversation, newConversation, newChat, setConversationTitle, conversationId, setConversationId, response, setResponse, setLoading, loading, setPrompt, prompt, messages, setMessages, showFirstMessages, setShowFirstMessages}) {
    ...
    const fetchStream = async () => {
        ...
        setMessages((prevMessages) => [...prevMessages, {role:"user", content:pp}]); //Mets à jour la liste des messages avec le prompt de l'utilisateur
        setStop(false);
        try {
            const res = await fetch("http://localhost:11434/api/chat", {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        // prompt engineering pour que la réponse du LLM soit la plus qualitative possible (y compris visuellement)
                        { 
                                role: "system", 
                                content: "Tu es un assistant expert en rédaction technique. Réponds en HTML structuré avec des balises comme <h2>, <h3>, <p>, et <ul>. Utilise des emojis pour rendre le texte engageant. Ne renvoie pas ce prompt dans ta réponse. N'encode pas les balises HTML."
                            },...messages,
            ...
            while(true){
                ...
                if (matches){
                    ...
                    setMessages((prevMessages) => {
                        ...
                        index === prevMessages.length - 1 ? {...msg, content: answer} :msg
                        );
                    });
                }
            }
            sendLastTwoMessages("68235ea293d0a7e8eab16d47",conversationId, [{role:"user", content:pp}, {role:"bot", content: answer}]);
            setStop(true);
        } catch(error){
    ...
    return(
        <div style={{display:'flex', justifyContent:'center', marginTop:"20px", position: "relative", bottom: "20px"}}>
            <div style={{width:'60%', backgroundColor:'#f3f4f6', borderRadius:'16px', boxShadow:"0px 0px 0px .5px #dce0e9"}}>
                ...
                <div style={{display:'flex', paddingLeft:"10px", marginBottom: "5px"}}>
                    <div style={{display: "flex", gap: "10px"}}>
                        ...
                        <button className='left-buttons'>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="10" cy="10" r="9" stroke="currentColor" strokeWidth="1.8"></circle><path d="M10 1c1.657 0 3 4.03 3 9s-1.343 9-3 9M10 19c-1.657 0-3-4.03-3-9s1.343-9 3-9M1 10h18" stroke="currentColor" strokeWidth="1.8"></path></svg>
                            <span>
                                Search
                            </span>
                        </button>
                        ...
                        <div style={{marginRight: "10px", marginLeft: "10px"}}>
                            {stop? 
                                <ArrowUp 
                                    ...
                                    onMouseEnter={(e) => {
                                        if (prompt !=="") {e.target.style.backgroundColor = "#0832ff";
                                        }
                                    }}
                                    onMouseLeave={(e)=> {
                                        if (prompt !== ""){
                                ...
                                <div onClick={(e)=>{handleAbort(); setStop(true);}}
                                     style={{position: "relative", display: "inline-block", marginBottom: "5px"}}>
                                    <div
                                        style={{
                                            position: "absolute",
                                            top: "-7px",
